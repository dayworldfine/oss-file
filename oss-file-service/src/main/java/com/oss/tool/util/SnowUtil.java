...
public class SnowUtil {
    ...
    private static SnowUtil snowFlakeUtil = new SnowUtil(0, 0);

    private SnowUtil(long datacenterId, long machineId) {
...
//                    SnowFlakeUtil ss = new SnowFlakeUtil(2, 3);
                    for (int j = 0; j < 1000; j++) {
                        Long k = SnowUtil.generateId();
                        if (!keys.contains(k)) {
                            keys.add(k);
                            System.out.println("不重复:" + k);
                        } else {
